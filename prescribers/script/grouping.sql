...
--groups by opiod flag combos (specialty/y/n, null/null/y/n)
...
--it changes focus of the grouping to the specialties (specialty/null/y/n, null/null)
...
--it is both rollup answers (all possible combos of the 2 specialties/2 flags/2 nulls)
...
SELECT 
	CASE WHEN drug_name LIKE '%CODEINE%' THEN 'codeine'
	WHEN drug_name LIKE '%FENTANYL%' THEN 'fentanyl'
	WHEN drug_name LIKE '%HYDROCODONE%' THEN 'hydrocodone'
	WHEN drug_name LIKE '%MORPHINE%' THEN 'morphine'
	WHEN drug_name LIKE '%OXYCODONE%' THEN 'oxycodone'
	WHEN drug_name LIKE '%OXYMORPHONE%' THEN 'oxymorphone'
	END AS drug_name
FROM drug
